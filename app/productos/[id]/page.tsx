...
import { getProductById, getProductsWithCategories } from "@/lib/sanity/queries"
...
export default async function ProductoPage({
  ...
  const imageUrl = product.image ? urlFor(product.image).width(800).height(800).url() : null

  // Obtener productos relacionados de la misma categoría
  const allProducts = await getProductsWithCategories()
  const relatedProducts = allProducts
    .filter((p) => p._id !== product._id && (p.category as any)?._id === category?._id)
    .slice(0, 4)
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-6 text-balance">{product.name}</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-auto">
              <WhatsAppButton message={WHATSAPP_MESSAGES.product(product.name)} size="lg" className="flex-1">
            ...
            <Card className="mt-8">
              <CardContent className="p-6">
                <h3 className="font-semibold text-lg mb-4">Características del producto</h3>
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((relatedProduct) => {
                ...
                return (
                  <Link key={relatedProduct._id} href={`/productos/${relatedProduct._id}`} className="group">
